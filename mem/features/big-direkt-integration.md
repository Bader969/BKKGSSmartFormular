...
- Eigene Mitgliedschaft → eigener Plusbonus: In Variante B wird pro Ehegatte/Kind mit `eigeneMitgliedschaft === true` (und Name/Vorname vorhanden) ein zusätzlicher Plusbonus-Antrag erzeugt. Person als Antragsteller (Vorname/Name/Geburtsdatum/Geschlecht der Person). Adresse = Mitglieds-Adresse. SEPA (`bigBank`), `bigVersicherungsstatus`, `bigHoeheEuro`, `bigVersicherungsarten`, `bigMitversicherte` (inkl. Chunking) und Unterschrift = identisch mit Hauptantrag. Reihenfolge der Exports: Mitglied → Ehegatte → Kinder. Kind-Geschlecht-Mapping: m→maennlich, w→weiblich, d/x→divers.
