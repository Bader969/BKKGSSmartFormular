...
- Eigene Mitgliedschaft → eigener Plusbonus (Auto-Trigger, kein UI-Toggle bei BIG): In Variante B wird pro Ehegatte/Kind automatisch ein zusätzlicher Plusbonus-Antrag erzeugt, sobald `bisherigArt === 'mitgliedschaft'` (oder Legacy-Flag `eigeneMitgliedschaft === true`) und Name/Vorname vorhanden. Person als Antragsteller (Vorname/Name/Geburtsdatum/Geschlecht der Person). Adresse = Mitglieds-Adresse. **Bankdaten/SEPA (`bigBank`) und Plusbonus-Unterschrift werden zentral aus dem Hauptantrag übernommen — eine einzige Bankverbindung und eine einzige Signatur (Nachname `bigBank.kontoinhaberNachname`) für alle Plusbonus-PDFs (Mitglied + Ehegatte + Kinder).** Pro Person individuell bleiben nur Status, Höhe in Euro und Versicherungsart (`eigenePlusbonus.versicherungsstatus/hoeheEuro/versicherungsarten`). **Mitversicherte Angehörige** (`bigMitversicherte`) werden NUR im Hauptmitglieds-PDF gefüllt; in den eigenen Plusbonus-PDFs für Ehegatte/Kinder bleiben diese Felder leer und es findet keine Chunk-Aufteilung statt (genau 1 PDF pro solcher Person). Reihenfolge der Exports: Mitglied → Ehegatte → Kinder. Kind-Geschlecht-Mapping: m→maennlich, w→weiblich, d/x→divers.
...
- Bei eigener Mitgliedschaft wird automatisch `FamilyMember.eigenePlusbonus` (nur Status/Höhe/Arten) initialisiert (Defaults aus Hauptmitglied). Bankdaten liegen ausschließlich zentral in `formData.bigBank`.
...
- Pro Person mit eigener Mitgliedschaft: separate `EigenePlusbonusBlock`-Sektion — **nur** Status, Höhe in Euro, Versicherungsarten (keine Bankdaten/SEPA-Felder; diese werden zentral aus dem Hauptantrag übernommen).
...
- `bigPlusbonusExport`: pro Ehegatte/Kind mit eigener Mitgliedschaft eigenes PDF mit Override aus `eigenePlusbonus` (nur Status, Höhe, Arten). Bank + Unterschrift = zentral aus `formData.bigBank` / `formData.unterschrift` (gilt für ALLE Plusbonus-PDFs).
...
- **Plusbonus-Antrag pro Person** (Variante B): Unterschrift = Nachname **Hauptkontoinhaber** (`bigBank.kontoinhaberNachname`) — identisch für alle Plusbonus-PDFs.
...
- Renders `MemberSection` (vollständig, wie andere Provider) + `BigPlusbonusSection` + `SignatureSection` (keine Spouse/Children).
- Required fields: Vorname, Name, Strasse, Hausnr., PLZ, Ort, Geschlecht (m/w/d), Bank: Kontoinhaber, IBAN, Ort, Datum (TTMMJJ on PDF), Mitglieds-Unterschrift. Kreditinstitut/BIC bleiben optional.
- Alle Mitgliedsfelder (Geburtsdatum, Geburtsort/-land, KV-Nummer, Krankenkasse, Familienstand, Telefon, E-Mail, Infoblock) sind sichtbar. Geburtsdatum bleibt in Variante A optional (Index-Validierung überspringt Pflicht für `big_plusbonus`).
...
- Auto-Fill „Mitversicherte Angehörige" im Plusbonus-Block: solange B aktiv ist, wird das `bigMitversicherte`-Array via `useEffect` in `Index.tsx` aus Familie synchronisiert. **Nur Familienversicherte** — Ehegatte/Kinder mit `eigeneMitgliedschaft===true` werden NIE in der Mitversicherten-Liste eingetragen (sie erhalten einen eigenen Plusbonus-Antrag). Reihenfolge: Ehegatte (falls familienversichert) zuerst, dann Kinder 1..N (nur familienversicherte). Format pro Eintrag: `"Nachname, Vorname"`.
- **Police-Höhen (Random + Auto-Summe)**: `hoehePolice` jeder Position wird einmalig mit einer ganzen Zufallszahl zwischen 200 und 245 vorbelegt (`src/utils/bigRandom.ts → randomPoliceBetrag`). Zusätzlich speichert `formData.bigHoeheEuroSelfRandom` einen einmalig gewürfelten Eigenanteil (ebenfalls 200–245) für das Hauptmitglied. Ein `useEffect` summiert beides (`policeSum + selfRandom`) und schreibt das Ergebnis in `formData.bigHoeheEuro` (AcroField „Euro"). Beispiel: 210+215+225+245 = 895. Felder bleiben manuell editierbar; bei Änderung greift die Summe automatisch.
